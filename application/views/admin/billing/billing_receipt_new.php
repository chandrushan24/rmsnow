...
<div class="card">
  ...
  <form id="myForm" class="card-body" method="post" enctype="multipart/form-data" onSubmit="return false">
    ...
    <div class="col-md-4" style="display:none;">
          ...
          <input type="hidden" class="form-control" name="billing_receipt_id" id="billing_receipt_id" value="<?php if(isset($get_prop)){echo $get_prop['billing_receipt_id'];}?>" />
      ...
      <div class="col-md-4">
          <div class="input-group input-group-merge">
            <div class="form-floating form-floating-outline">
              <input type="text" id="multicol-confirm-total_register_price_input" class="form-control" name="total_register_price" value="<?php if(isset($get_prop)){echo $get_prop['total_register_price'];}?>" placeholder="Total Register Amount"  />
              <label for="multicol-confirm-total_register_price_input">Total Register Amount</label>
            </div>
          </div>
      </div>
      ...
      <div class="col-md-4">
       ...
       <div class="col-md-4">
          <div class="input-group input-group-merge">
            <div class="form-floating form-floating-outline">
              <input type="text" id="total_payment_received" class="form-control" name="total_payment_received" value="<?php if(isset($get_prop)){echo $get_prop['total_payment_received'];}?>" placeholder="Total Register Amount"  />
              <label for="multicol-confirm-total_payment_received_input">Total Payment Received</label>
            </div>
          </div>
      </div>
...
function get_offer_incentive(property_id){
    ...
    var cus_id = $('#customer_name_input').val();
    prev_cusid = cus_id;

    if($("#billing_receipt_id").val() =="" || cus_id != prev_cusid)
    {

    if(property_id != '' && property_id != null && payment_mode == 'Cash'){
    $.ajax({
          url: '<?=base_url()?>admin/Billing_receipt_controller/get_balance_registerted_amount?property_id='+property_id+'&cus_id='+cus_id,
          type: 'GET',
          processData: false,  
          contentType: false, 
          dataType: 'json',  
          ...
          if(response && response.length >0){
            ...
            var previous_payment_amount = parseFloat(response[0].total_payment_received);
         
            $('#total_payment_received').val((response[0].total_payment_received));

          //  $("#multicol-confirm-total_register_price_input").val((response[0].total_plot_price));
            ...
            // var total_values = $("#multicol-confirm-total_register_price_input").val();
            // total_values = total_values ? parseFloat(total_values) : 0;

            // total_values = total_values - (parseFloat(amount_adv) + parseFloat(amount_paying));
           
            // var bal_amount = (total_values);

            // $('#bal_amount_plot_input').val(bal_amount);

            calculateBalance(previous_payment_amount);
  ...
  $(document).on("keyup","#mode_payment_value_input",function(){
    calculateBalance();
  })

  function calculateBalance(previous_payment_amount)
  {
            var amount_adv =  $("[name='adv_amount_plot_input']").val();
            amount_adv = amount_adv ? parseFloat(amount_adv) : 0;

            var total_payment_received =  $("[name='total_payment_received']").val();
            total_payment_received = total_payment_received ? parseFloat(total_payment_received) : 0;
            ...
            total_values = total_values - (parseFloat(amount_adv) + parseFloat(amount_paying) + parseFloat(total_payment_received));
...
function trigger_values_sno_customer(id,status){
    if(id){
      $.ajax({
          ...
          $("#multicol-confirm-total_register_price_input").val(plot.total_plot_price);
